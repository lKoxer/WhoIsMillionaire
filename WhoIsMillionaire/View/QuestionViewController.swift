...
//        label.font = UIFont(name: "Roboto-Medium", size: 30)
    ...
    func setCurrentlabelImage(){
        ...
        if questionCounter > 5 && questionCounter <= 10, safeSum == 1000 {
            buttonPrice5.setBackgroundImage(UIImage(named: "waitingAnswerImage"), for: .normal)
            buttonQuestion5.setBackgroundImage(UIImage(named: "waitingAnswerImage"), for: .normal)
            questionCounter = 1000 // рандомное число чтобы кнопки из массива ниже не меняли цвет
            wonSumLabel.text = buttonPrice5.currentTitle
        }
        
        if questionCounter > 10 && questionCounter <= 14, safeSum == 32000 {
            buttonPrice10.setBackgroundImage(UIImage(named: "waitingAnswerImage"), for: .normal)
            buttonQuestion10.setBackgroundImage(UIImage(named: "waitingAnswerImage"), for: .normal)
            questionCounter = 1000
            wonSumLabel.text = buttonPrice10.currentTitle
        }
        
        if questionCounter == 15 , safeSum == 100000 {
            buttonPrice10.setBackgroundImage(UIImage(named: "waitingAnswerImage"), for: .normal)
            buttonQuestion10.setBackgroundImage(UIImage(named: "waitingAnswerImage"), for: .normal)
            questionCounter = 1000
        ...
        for (index, element) in [buttonQuestion1, buttonQuestion2, buttonQuestion3, buttonQuestion4, buttonQuestion5, buttonQuestion6, buttonQuestion7, buttonQuestion8, buttonQuestion9, buttonQuestion10, buttonQuestion11, buttonQuestion12, buttonQuestion13, buttonQuestion14, buttonQuestion15].enumerated() {
            if index == questionCounter, safeSum != 0 {
        ...
        for (index, element) in [buttonPrice1, buttonPrice2, buttonPrice3, buttonPrice4, buttonPrice5, buttonPrice6, buttonPrice7, buttonPrice8, buttonPrice9, buttonPrice10, buttonPrice11, buttonPrice12, buttonPrice13, buttonPrice14, buttonPrice15].enumerated() {
            if index == questionCounter,  safeSum != 0 {
                element.setBackgroundImage(UIImage(named: "waitingAnswerImage"), for: .normal)
                wonSumLabel.text = element.currentTitle
            }
        }
...
//            view.addSubview(wonSumLabel)
            ...
            NSLayoutConstraint.activate ([
                ...
                wonSumLabel.topAnchor.constraint(equalTo: logoButton.bottomAnchor, constant: 6),
                wonSumLabel.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: 16),
                wonSumLabel.trailingAnchor.constraint(equalTo: view.trailingAnchor, constant: -16),
                wonSumLabel.heightAnchor.constraint(equalToConstant: 60),
                
                
                mainStackView.topAnchor.constraint(equalTo: wonSumLabel.bottomAnchor, constant: 16),
